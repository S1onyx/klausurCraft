...
public class PdfExportService {
    ...
    public boolean export(Window owner, String title, LocalDate date,
        ...
        File selected = chooseSaveTarget(owner, "exam_" + ts + ".pdf", p.get(PREF_LAST_EXPORT_DIR, null));
        if (selected == null) return false;
        File examFile = PdfExportRules.ensurePdfExtension(selected);
        ...
        if (withSolution) {
            File solFile = PdfExportRules.buildSolutionFile(examFile);
    ...
    /**
     * UI boundary for selecting the target PDF file.
     * Extracted for testability: unit tests can override this method and bypass JavaFX dialogs.
     */
    protected File chooseSaveTarget(Window owner, String suggestedName, String lastDir) {
    ...
    private void addExamCover(Document doc, String title, LocalDate date, List<TaskAssembly> tasks,
        ...
        BigDecimal totalPoints = PdfExportRules.totalPoints(tasks);
        ...
        info.addCell(valueCell(PdfExportRules.formatDate(date), f.normal));
    ...
    private void addSolutionCover(Document doc, String title, LocalDate date, List<TaskAssembly> tasks, PdfFonts f) throws Exception {
        BigDecimal total = PdfExportRules.totalPoints(tasks);
    ...
    private void addTeacherOverview(Document doc, List<TaskAssembly> tasks, PdfFonts f) throws Exception {
        ...
        for (TaskAssembly ta : tasks) {
            BigDecimal max = PdfExportRules.taskPoints(ta);
            total = total.add(max);
    ...
    private void addSubtaskBlock(Document doc, PdfWriter writer, int taskNumber, int idx, ChosenVariant entry,
        ...
        if (includeSolutions) {
            ...
            cell.setPadding(6f);
            cell.setMinimumHeight(PdfExportRules.answerBoxHeight(st));
    ...
    private void ensureSpaceForSubtask(Document doc, PdfWriter writer, Subtask st, String text, boolean includeSolutions) {
        ...
        float needed = base + (includeSolutions ? 72f : PdfExportRules.answerBoxHeight(st) + 12f);
